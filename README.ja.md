...
OMC が専門エージェントとワークフロー自動化によって Claude Code を拡張したように、OMG は VS Code の Copilot agent mode 上で同じ発想を実装します。ECC 統合（v1.1.0）と OMC スキルの OMG-native な適応移植により、OMG は言語特化レビュアー、TDD 強制、高速セキュリティスキャン、`ultragoal` チェックポイント、研究補助、リリースガイド、ビジュアル QA などを取り込んでいます。1つのアシスタントにすべてを任せるのではなく、**28 の専門エージェント** と **37 の再利用可能なスキル** を MCP サーバー経由で連携させ、計画、実装、レビュー、検証を構造化して進めます。
...
OMG now includes 37 skills, including OMG-adapted OMC-inspired workflows such as `ultragoal`, `visual-verdict`, `release`, `deepinit`, `skillify`, `writer-memory`, `configure-notifications`, `wiki`, `autoresearch`, `hud`, `external-context`, `sciomc`, and `ask`.
...
| スキル | 10+ のワークフロースキル | 37 スキルとキーワードトリガー |
...
### v1.4.4 (2026-05-16) — OMC スキル適応移植 + Ultragoal MCP

**高価値な OMC ワークフローを VS Code/Copilot 向けの OMG-native 形式で移植しました。** OMG は 37 スキルに拡張され、MCP ベースの durable goal tracking を追加しました。

- `ultragoal` MCP ツール: `omg_ultragoal_create`, `omg_ultragoal_status`, `omg_ultragoal_checkpoint`, `omg_ultragoal_complete`。
- 新規 13 スキル: `visual-verdict`, `release`, `deepinit`, `skillify`, `writer-memory`, `configure-notifications`, `ultragoal`, `wiki`, `autoresearch`, `hud`, `external-context`, `sciomc`, `ask`。
- Copilot instructions、keyword triggers、hook gates、skill threshold (`CORE=24`, `EXTENDED=37`) を更新。
- 検証: MCP 23 tests、VS Code extension 29 tests、VSIX <20MB。
...
- **階層的エージェント/スキル閾値**: agents warn <20 / info <28 / silent ≥28, skills warn <24 / info <37 / silent ≥37
